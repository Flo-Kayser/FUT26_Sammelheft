<script>
	...
	import { totsIndexStore } from '$lib/stores/sessionStores';
	...
	onMount(() => {
		apiClient.fetchData('index-data/batches/index-tots-batches.json').then(totsIndexStore.set);
	});
...
<div class="flex flex-col gap-8">
	{#if Array.isArray($totsIndexStore)}
		{#each $totsIndexStore as batch}
			<button
