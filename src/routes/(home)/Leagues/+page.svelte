...
<section class="grid grid-cols-1 gap-4 px-4 py-4">
	{#each filteredLeagues as league, i}
		...
		>
				...
				<div class="flex items-center h-full gap-4">
					...
					<img
						...
						on:error={(e) => {
							...
							img.src = `https://cdn.easysbc.io/fc26/leagues/${league.id}.png`;
							img.onerror = null; 
						}}
					/>
			...
			<!-- Expand-Pfeil -->
			<button
				class="absolute top-1/2 cursor-pointer -translate-y-1/2 left-2 text-textC transition-all duration-500 {expandedLeagueId ===
				league.id
					? 'rotate-90'
					: ''}"
					...
					{#if !(league.id == 2118 && String(cid.id).includes('_'))}
						...
						>
							<div class="flex h-full items-center gap-2 px-2">
								...
								{:else}
									<img
										src={`https://cdn.easysbc.io/fc26/clubs/${cid.id}.png`}
										class="h-full contrast-75"
										alt=""
									/>
								{/if}
